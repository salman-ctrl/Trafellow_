...
export default function EditRegion() {
  ...
  const fetchRegion = async () => {
    try {
      ...
      if (region.image) {
        setImagePreview(getImageUrl(region.image));
      }
    } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await api.put(`/regions/${id}`, submitData, {
        headers: {
      ...
      const message = error.response?.data?.message || 'Gagal mengupdate region';
      toast.error(message);
    } finally {
      setSubmitting(false);
    }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center space-x-4">
        ...
        <div>
          <h1 className="text-4xl font-bold text-gray-800">Edit Region</h1>
          <p className="text-gray-600">Update region information</p>
        </div>
      </div>
